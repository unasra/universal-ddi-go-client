...
Other parameters are passed through a pointer to a `ViewAPIBulkCopyRequest` struct via the builder pattern
...
func main() {
	...
	apiClient := dnsconfig.NewAPIClient()
	resp, r, err := apiClient.ViewAPI.Read(context.Background(), id).Execute()
	if err != nil {

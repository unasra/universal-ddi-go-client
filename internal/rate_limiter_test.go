...
func TestNewConfigurationDefaultRetry(t *testing.T) {
	...
	assert.Equal(t, DefaultRetryAttempts, cfg.RetryConfig.MaxAttempts)
...
func TestNewConfigurationRetryDisabledByEnv(t *testing.T) {
	t.Setenv("INFOBLOX_RETRY_ATTEMPTS", "0")
	cfg := NewConfiguration()
...
func TestRetryBackoff(t *testing.T) {
	...
	for attempt := 0; attempt < 5; attempt++ {
		...
		assert.GreaterOrEqual(t, wait, minWait, "backoff should not be less than minWait")
		assert.LessOrEqual(t, wait, maxWait+maxWait/4, "backoff should not exceed maxWait + jitter")
	}
}
...
func TestRetryBackoffExponentialGrowth(t *testing.T) {
	...
	// Run multiple times and average to smooth out jitter
	samples := 100
...
// --- CallAPI retry integration tests ---

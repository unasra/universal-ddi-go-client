...
func TestCallAPIRateLimiterContextCancelled(t *testing.T) {
	...
	// Limiter with zero rate — Wait will block until context is cancelled
	cfg := &Configuration{
		HTTPClient:  server.Client(),
...
func TestCallAPIRateLimiterThroughput(t *testing.T) {
	...
	// With burst=1, the first request goes through immediately,
	// then the remaining 9 are rate-limited at 20/s ≈ 450ms minimum
